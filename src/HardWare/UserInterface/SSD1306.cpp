#include<HardWare/UserInterface/Modules/SSD1306.h>
...
namespace SPI{
    ...
    void SSD1306::Refresh(){
        for(u_char i=0;i<this->height;i++){
            SSD1306_Cmd(0xB0+i);
            SSD1306_Cmd(0x00);
            SSD1306_Cmd(0x10);
            for(u_char n=0;n<this->width;n++){
                SSD1306_Dat(GRAM[n][i]);
            }
        }
