...
namespace HardWare{
    ...
    MPU6050::MPU6050(uint32_t SDA_GPIOx,uint32_t SDA_PINx,uint32_t SCL_GPIOx,uint32_t SCL_PINx):I2C_Component(SDA_GPIOx,SDA_PINx,SCL_GPIOx,SCL_PINx){
        ...
        this->Send_Reg(USEREG::MPU_PWR_MGMT1_REG,0x80);
    ...
    void MPU6050::Set_Gyro_Fsr(Gyro_Fsr Fsr){
        ...
        uint32_t timeout=0;
        do{
            this->Send_Reg(USEREG::MPU_GYRO_CFG_REG,reg);
            reg=this->Read_Reg(USEREG::MPU_GYRO_CFG_REG);
    ...
    void MPU6050::Set_Accel_Fsr(Accel_Fsr Fsr){
        ...
        uint32_t timeout=0;
        do{
            this->Send_Reg(USEREG::MPU_ACCEL_CFG_REG,reg);
            reg=this->Read_Reg(USEREG::MPU_ACCEL_CFG_REG);
        }while(reg!=(u_char)Fsr&&++timeout<=1000);
        if(timeout>=1000){
    ...
    void MPU6050::Set_LPF(u_short Lpf){
        ...
        uint32_t timeout=0;
        do{
            this->Send_Reg(USEREG::MPU_CFG_REG,reg);
            reg=this->Read_Reg(USEREG::MPU_CFG_REG);
    ...
    void MPU6050::Set_Rate(u_short Rate){
        ...
        uint32_t timeout=0;
        do{
            this->Send_Reg(USEREG::MPU_SAMPLE_RATE_REG,reg);
            reg=this->Read_Reg(USEREG::MPU_SAMPLE_RATE_REG);
        }while(reg!=data&&++timeout<=1000);
        if(timeout>=1000){
    ...
    void MPU6050::Refresh_Temperature(){
        u_char buf[2]={0}; 
        if(this->Read_Len(USEREG::MPU_TEMP_OUTH_REG,2,buf)){
    ...
    void MPU6050::Refresh_Gyroscope(){
        u_char buf[6]={0};  
        if(this->Read_Len(USEREG::MPU_GYRO_XOUTH_REG,6,buf)){
    ...
    void MPU6050::Refresh_Accelerometer(){
        u_char buf[6]={0};  
        if(this->Read_Len(USEREG::MPU_ACCEL_XOUTH_REG,6,buf)){
    ...
    void MPU6050::WeakUp(){
        ...
        uint32_t timeout=0;
        do{
            this->Send_Reg(USEREG::MPU_PWR_MGMT1_REG,reg);
            reg=this->Read_Reg(USEREG::MPU_PWR_MGMT1_REG);
        ...
        this->Send_Reg(USEREG::MPU_PWR_MGMT1_REG,0X00);
    }

    void MPU6050::ID_Check(){
        u_char res=this->Read_Reg(USEREG::MPU_DEVICE_ID_REG); 
        if(res==this->address){
